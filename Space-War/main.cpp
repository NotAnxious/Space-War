...
int main() {
	...
	// 初始化变量
	HDC hdc = GetDC(NULL);
	auto Tstart = chrono::high_resolution_clock::now();// 计时器
	auto Tend = chrono::high_resolution_clock::now();// 计时器
	...
	while (true) {
		Tstart = chrono::high_resolution_clock::now();// 开始计时
		...
		Tend = chrono::high_resolution_clock::now();// 结束计时
		// 计算时间
		chrono::duration<double, milli> duration = Tend - Tstart;
		if (int(duration.count()) < 17) {
			cout << 17 - int(duration.count())<<endl;
			Sleep(17 - int(duration.count()));
		}
	}
}